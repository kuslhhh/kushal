...
import { FileTextIcon, FileIcon, PlusIcon, ImageIcon } from "@radix-ui/react-icons";

export default async function AdminDashboard() {
    const [blogCount, photoCount, activeResume] = await Promise.all([
    ...
    return (
        <div className={`${bricolage_grotesque}`}>
            <h1 className="text-3xl font-bold dark:text-white text-black mb-1">Dashboard</h1>
            ...
            <div className="grid grid-cols-3 max-sm:grid-cols-1 gap-3 max-w-xl mb-10">
                <div className="border dark:border-white/10 border-black/10 rounded-xl p-5">
                    ...
                    <p className="text-4xl font-bold dark:text-white text-black">{blogCount}</p>
                </div>
                <div className="border dark:border-white/10 border-black/10 rounded-xl p-5">
                    <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
                        <ImageIcon className="w-4 h-4" /> Photos
                    </div>
                    <p className="text-4xl font-bold dark:text-white text-black">{photoCount}</p>
                </div>
                <div className="border dark:border-white/10 border-black/10 rounded-xl p-5">
                    <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
                    ...
                    <p className="text-sm font-semibold dark:text-white text-black mt-1 truncate">
                        {activeResume ? activeResume.label : "None"}
                    </p>
                </div>
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
                ...
                >
                    <PlusIcon className="w-4 h-4" /> New Blog Post
                </Link>
                <Link
                    href="/admin/photos"
                ...
                <Link
                    ...
                    className="flex items-center gap-2 border dark:border-white/20 border-black/20 px-4 py-2.5 rounded-lg text-sm font-semibold dark:text-white text-black hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                >
                    <FileIcon className="w-4 h-4" /> Update Resume
                </Link>
            </div>
        </div>
    );
